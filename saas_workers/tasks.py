"""
Celery Tasks for Background Processing

Production-ready async tasks for reconciliation jobs.
"""

import sys
import json
from pathlib import Path
from datetime import datetime
import subprocess
import logging

from .celery_app import celery_app
from saas_config.database import SessionLocal
from saas_models import ReconciliationJob, JobStatus, Invoice, SourceType, AuditLog

logger = logging.getLogger(__name__)


@celery_app.task(bind=True, name='saas_workers.tasks.process_reconciliation')
def process_reconciliation(self, job_id: str, tenant_id: str):
    """
    Process a reconciliation job asynchronously.
    
    This is a Celery task that runs in a worker process.
    
    Args:
        job_id: UUID of the job to process
        tenant_id: UUID of the tenant (for isolation)
    
    Returns:
        dict: Job result summary
    """
    db = SessionLocal()
    
    try:
        # Update task state
        self.update_state(state='PROCESSING', meta={'job_id': job_id})
        
        # Get job
        job = db.query(ReconciliationJob).filter(
            ReconciliationJob.id == job_id,
            ReconciliationJob.tenant_id == tenant_id
        ).first()
        
        if not job:
            raise ValueError(f"Job {job_id} not found")
        
        # Update status
        job.status = JobStatus.PROCESSING
        job.started_at = datetime.utcnow()
        db.commit()
        
        logger.info(f"Processing job {job_id} for tenant {tenant_id}")
        
        # Process EVD file
        evd_output = Path(job.evd_file_path).parent / "evd_data.json"
        result = subprocess.run([
            sys.executable,
            '-m',
            'evd_extraction_project.evd_extractor',
            job.evd_file_path,
            str(evd_output)
        ], capture_output=True, text=True, timeout=60)
        
        if result.returncode != 0:
            raise Exception(f"EVD processing failed: {result.stderr}")
        
        # Load EVD data
        with open(evd_output, 'r', encoding='utf-8') as f:
            evd_data = json.load(f)
        
        # Process PDF file
        pdf_output = Path(job.pdf_file_path).parent / "pdf_data.json"
        result = subprocess.run([
            sys.executable,
            '-m',
            'pdf_extraction_project.pdf_processor',
            job.pdf_file_path,
            str(pdf_output)
        ], capture_output=True, text=True, timeout=120)
        
        if result.returncode != 0:
            raise Exception(f"PDF processing failed: {result.stderr}")
        
        # Load PDF data
        with open(pdf_output, 'r', encoding='utf-8') as f:
            pdf_data = json.load(f)
        
        # Run reconciliation
        from reconciliation_project.reconciliation_report import ReconciliationReportGenerator
        
        report_output = Path(job.evd_file_path).parent / f"report_{job_id}.xlsx"
        generator = ReconciliationReportGenerator()
        generator.generate_report(evd_data, pdf_data, report_output)
        
        # Store results in job
        job.evd_data = evd_data
        job.pdf_data = pdf_data
        job.report_file_path = str(report_output)
        
        # Extract summary statistics
        evd_invoices = evd_data.get('invoices', [])
        pdf_invoices = pdf_data.get('invoices', [])
        
        job.evd_invoice_count = len(evd_invoices)
        job.pdf_invoice_count = len(pdf_invoices)
        
        # Store invoices in database for searching
        _store_invoices(db, job, evd_invoices, pdf_invoices)
        
        # Calculate match statistics
        # TODO: Extract from reconciliation result
        job.matches_count = 0
        job.mismatches_count = 0
        job.match_rate = 0.0
        
        # Update status
        job.status = JobStatus.COMPLETED
        job.completed_at = datetime.utcnow()
        
        # Audit log
        audit = AuditLog(
            tenant_id=job.tenant_id,
            user_id=job.user_id,
            action="job.completed",
            resource_type="job",
            resource_id=job.id,
            details={
                "evd_count": job.evd_invoice_count,
                "pdf_count": job.pdf_invoice_count
            }
        )
        db.add(audit)
        
        db.commit()
        logger.info(f"Job {job_id} completed successfully")
        
        return {
            'job_id': str(job_id),
            'status': 'completed',
            'evd_count': job.evd_invoice_count,
            'pdf_count': job.pdf_invoice_count
        }
        
    except Exception as e:
        logger.error(f"Job {job_id} failed: {str(e)}", exc_info=True)
        
        job.status = JobStatus.FAILED
        job.error_message = str(e)
        job.completed_at = datetime.utcnow()
        
        # Audit log
        audit = AuditLog(
            tenant_id=job.tenant_id,
            user_id=job.user_id,
            action="job.failed",
            resource_type="job",
            resource_id=job.id,
            details={"error": str(e)}
        )
        db.add(audit)
        
        db.commit()
        
        # Retry logic
        if self.request.retries < 3:
            raise self.retry(exc=e, countdown=60 * (self.request.retries + 1))
        
        return {
            'job_id': str(job_id),
            'status': 'failed',
            'error': str(e)
        }
        
    finally:
        db.close()


def _store_invoices(db, job, evd_invoices: list, pdf_invoices: list):
    """Store invoices in database for searching."""
    
    # Store EVD invoices
    for evd in evd_invoices:
        invoice = Invoice(
            tenant_id=job.tenant_id,
            job_id=job.id,
            invoice_number=evd.get('invoice_number', ''),
            vendor_name=evd.get('vendor', ''),
            vendor_normalized=evd.get('vendor_normalized', ''),
            total_amount_eur=evd.get('total_amount_eur'),
            total_amount_bgn=evd.get('total_amount_bgn'),
            source_type=SourceType.EVD,
            source_file=job.evd_file_path,
            metadata=evd
        )
        db.add(invoice)
    
    # Store PDF invoices
    for pdf in pdf_invoices:
        invoice = Invoice(
            tenant_id=job.tenant_id,
            job_id=job.id,
            invoice_number=pdf.get('invoice_number', ''),
            vendor_name=pdf.get('vendor', ''),
            vendor_normalized=pdf.get('vendor_normalized', ''),
            total_amount_eur=pdf.get('total_amount_eur'),
            net_amount_eur=pdf.get('net_amount_eur'),
            vat_amount_eur=pdf.get('vat_amount_eur'),
            source_type=SourceType.PDF,
            source_file=job.pdf_file_path,
            extraction_method=pdf.get('extraction_method'),
            confidence=pdf.get('confidence'),
            metadata=pdf
        )
        db.add(invoice)


@celery_app.task(name='saas_workers.tasks.cleanup_old_files')
def cleanup_old_files():
    """
    Periodic task to clean up old files.
    
    Run daily to remove files older than retention period.
    """
    from datetime import timedelta
    from saas_config import settings
    
    logger.info("Starting file cleanup task")
    
    retention_days = 30  # Keep files for 30 days
    cutoff_date = datetime.utcnow() - timedelta(days=retention_days)
    
    db = SessionLocal()
    try:
        # Find old completed/failed jobs
        old_jobs = db.query(ReconciliationJob).filter(
            ReconciliationJob.completed_at < cutoff_date,
            ReconciliationJob.status.in_([JobStatus.COMPLETED, JobStatus.FAILED])
        ).all()
        
        deleted_count = 0
        for job in old_jobs:
            # Delete files
            if job.evd_file_path:
                Path(job.evd_file_path).unlink(missing_ok=True)
            if job.pdf_file_path:
                Path(job.pdf_file_path).unlink(missing_ok=True)
            if job.report_file_path:
                Path(job.report_file_path).unlink(missing_ok=True)
            
            # Clear file paths from database (keep metadata)
            job.evd_file_path = None
            job.pdf_file_path = None
            job.report_file_path = None
            
            deleted_count += 1
        
        db.commit()
        logger.info(f"Cleanup complete: {deleted_count} jobs processed")
        
        return {'deleted_count': deleted_count}
        
    finally:
        db.close()


@celery_app.task(name='saas_workers.tasks.reset_monthly_limits')
def reset_monthly_limits():
    """
    Reset tenant monthly job limits.
    
    Run on the 1st of each month.
    """
    logger.info("Resetting monthly job limits")
    
    db = SessionLocal()
    try:
        from saas_models import Tenant
        
        tenants = db.query(Tenant).all()
        
        for tenant in tenants:
            tenant.jobs_this_month = 0
        
        db.commit()
        logger.info(f"Reset limits for {len(tenants)} tenants")
        
        return {'tenants_updated': len(tenants)}
        
    finally:
        db.close()


# Periodic task schedule
celery_app.conf.beat_schedule = {
    'cleanup-old-files-daily': {
        'task': 'saas_workers.tasks.cleanup_old_files',
        'schedule': 86400.0,  # Daily (24 hours)
    },
    'reset-monthly-limits': {
        'task': 'saas_workers.tasks.reset_monthly_limits',
        'schedule': {
            'month_of_year': '*',
            'day_of_month': '1',
            'hour': '0',
            'minute': '0',
        },
    },
}
